...
export class TicketController {
    cargar() {
        const id = localStorage.getItem("ultimoPedido");
        if (!id) {
            alert("No hay pedido para mostrar");
            return;
        }
        const ticket = document.getElementById("ticket");
        if (!ticket)
            return;
        onSnapshot(doc(db, "pedidos", id), (snapshot) => {
            ...
            document
                .getElementById("btnCancelarCompra")
                ?.addEventListener("click", () => {
        ...
        const btn = document.getElementById("btnAgregarMas");
